...
def parse_log_line(line):
    """
    Parse a log line that is now comma-separated from NetPacketCsvLogger.
    Columns: client_id,snapshot_id,seq_num,server_timestamp_ms,recv_time_ms,latency_ms,jitter_ms,posError,cpu,bw...
    """
    line = line.strip()
    if not line or line.startswith("client_id"):
        return None
    ...
    try:
        return {
            ...
            "perceived_position_error": float(parts[7]),
            "bandwidth_per_client_kbps": float(parts[9])
        }
    except Exception:
        return None
...
def main():
    ...
    # Recursively find all client_metrics.csv files
    log_files = list(results_dir.rglob("*/client_metrics.csv"))
    if not log_files:
...
def plot_comparisons(summary_csv, results_dir):
    ...
    def get_label(fname):
        if "baseline" in fname.lower(): return "Baseline"
        if "loss_2" in fname.lower(): return "Loss 2%"
        if "loss_5" in fname.lower(): return "Loss 5%"
        if "delay_100ms" in fname.lower(): return "Delay 100ms"
        return fname
